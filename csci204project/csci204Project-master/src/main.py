...
def user_interface():
    ...
    if os.path.basename(tpath) != 'train':
    ...
    #Check if proper path is inputted
...
def topMenu(info):
    ...
    print("5. Build Tree /Predict with SKTree")
    print("6. Build Tree/Predict with MyTree")
    print("7. Exit")
    t = int(input("?"))
    ...
    if t == 1:
        ...
        print('\n')
        print('Try again. Pick an available option.')
        print('\n')
        topMenu(info)
...
def topicAnalysisTrain(info):
    ...
    """
    #tic = time.time() #used to check efficiency issue
    exclude = set(('!', '.', '?'))
    freq = Stats()
    fullText = []
    #Parse emails
...
def predictMyTree(info):
    """
    Predicts the from info in from the emails in the eval folder
    """
    ...
    #Train the data, create a tree
    myTree.train(train2DL, train1DL, maxDepth)
    #Predict the data
    predict = myTree.eval(eval2DL)
    
    #myTree is returning strang list so extract integers corresponding to predictions
    predictL = []
    num = ''
    for i in range(len(str(predict))):
        if str(predict)[i].isnumeric() and not str(predict)[i+1].isnumeric():
            num += str(predict)[i]
            predictL.append(int(num))
            num = ''
        elif str(predict)[i].isnumeric():
            num += str(predict)[i]
    ...
    count = 0
    for directory in os.listdir(info.epath):
        print('Email ', directory , ' in eval folder is from ', fromEmails[predictL[count]-1])
        count += 1
